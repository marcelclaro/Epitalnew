...
#ifndef SOLUTIONWANNIER_HPP_
#define SOLUTIONWANNIER_HPP_
 ...
 */
template<typename complextype, typename xType>
class SolutionWannier{
public:
	SolutionWannier();
	virtual ~SolutionWannier();

	//SolutionWannier(string filename);
	 ...
	 */
	void addLevel(shared_ptr<WannierFunction<complextype,xType>> wavefunction,std::complex<complextype> energy);
	 ...
	 */
	shared_ptr<WannierFunction<complextype,xType>> getWavefunction(long int number);
...
protected:
	...
	list<shared_ptr<WannierFunction<complextype,xType>>> waves; ///<Pointers to wavefunctions
	list<std::complex<complextype>> energies; ///< Energies of founded wavefunctions

};

template<typename complextype, typename xType>
SolutionWannier<complextype,xType>::SolutionWannier(): levels(0){
}

template<typename complextype, typename xType>
SolutionWannier<complextype,xType>::~SolutionWannier(){

}


template<typename complextype, typename xType>
void SolutionWannier<complextype,xType>::addLevel(shared_ptr<WannierFunction<complextype,xType>> wavefunction,std::complex<complextype> energy){
	if(energies.size()==0){
...
template<typename complextype, typename xType>
shared_ptr<WannierFunction<complextype,xType>> SolutionWannier<complextype,xType>::getWavefunction(long int number){
	if(number >= levels)
...
template<typename complextype, typename xType>
std::complex<complextype> SolutionWannier<complextype,xType>::getEnergy(long int number){
	if(number > levels)
...
template<typename complextype, typename xType>
long int SolutionWannier<complextype,xType>::getLevels(){
	return levels;
}
/*
template<typename complextype, typename xType>
